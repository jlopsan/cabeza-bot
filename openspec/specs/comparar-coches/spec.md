...
Define el comando `/comparar` que enfrenta dos coches a nivel modelo — vía URLs de Wallapop/Coches.net, modelos en lenguaje natural, o mezcla — y devuelve un veredicto estructurado con ganador en precio, DGT, fiabilidad y TCO. Pipeline paralelo por lado con slot-filling multi-turno. Archivos: [comparar_pipeline.py](../../../comparar_pipeline.py), [main.py](../../../main.py) (handler cmd_comparar, líneas 2195-2307), [ai.py](../../../ai.py) (parsear_comparar_input, enriquecer_modelo, generar_veredicto_comparar).
...
### Requirement: Resolución de código de generación a años
Cuando un lado tiene `generacion` no vacía pero `año_central=0` tras el parser determinista de años (regex `19xx`/`20xx`), el bot MUST hacer una mini-llamada LLM vía `resolver_generacion_años(marca, modelo, generacion)` para resolver el código (Mk7, FK7, B9…) a rango de años. Los lados pendientes se resuelven en paralelo.

#### Scenario: Código de generación sin años implícitos
- **WHEN** el parser extrae `generacion="Mk7"` para Golf pero `año_central=0`
- **THEN** `resolver_generacion_años("Volkswagen", "Golf", "Mk7")` devuelve `(2013, 2019)` y el pipeline continúa sin preguntar al usuario
...
El procesador de cada lado MUST calcular sin llamar al LLM los siguientes derivados, usando `COMPARAR_KM_AÑO_REF` y `COMPARAR_PRECIO_LITRO_EUR` de `config.py`:
- `coste_combustible_anual_eur = round(consumo * KM_AÑO_REF / 100 * PRECIO_LITRO)` si `consumo > 0`
...
- **THEN** TCO_3a calculado por Python con las constantes de config, NO por el LLM
...
### Requirement: Veredicto IA con alias y nombres completos diferenciados
El veredicto MUST llamar a `generar_veredicto_comparar(a, b)`. La IA SHALL usar nombres completos (`nombre_display`) en cabecera y bullets de datos, y alias cortos (modelo + gen corta + versión corta) en resumen rápido, trofeos 🏆 y párrafos narrativos.

#### Scenario: Render del veredicto
- **WHEN** el veredicto se genera con `nombre_display="Volkswagen Golf Mk7 GTI"` y `alias="Golf Mk7 GTI"`
- **THEN** el HTML usa nombre completo en cabecera 🆚 y bullets de sección, y alias en resumen rápido y trofeos
...
El handler de `/comparar` MUST usar `@requiere_acceso("/comparar", registrar=False)` para chequear acceso sin descontar al inicio. SHALL llamar `registrar_uso(user_id, 1)` solo si el veredicto HTML se entrega con éxito al usuario. En timeout, excepción o veredicto vacío, NO descuenta crédito.
...
- **THEN** `guardar_historico_batch` los inserta en `historico_precios` aplicando los filtros del spec de [[dataset-historico]]
